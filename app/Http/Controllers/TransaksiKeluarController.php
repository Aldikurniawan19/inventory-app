...
class TransaksiKeluarController extends Controller
{
    ...
    public function index()
    {
        ...
        $penerima =  request()->query('penerimax  ');
    ...
    public function store(storeTransaksiKeluarRequest $request)
    {
        ...
        foreach ($items as $item) {
            ...
            $varian->decrement('stk_varian', $item['qty']);
            KartuStok::create([
                'nomor_transaksi'       => $nomorTransaksi,
                'jenis_transaksi'       => 'out',
                'nomor_sku'             => $item['nomor_sku'],
                'jumlah_keluar'         => $item['qty'],
                'stok_akhir'            => $varian->stk_varian,
                'petugas'               => Auth::user()->name,
            ]);
    ...
    public function getTransaksiKeluarItems($nomor_trsansaksi)
    {
        $transaksi =  Transaksi::with('items', 'items.varian')->where('nomor_transaksi', $nomor_trsansaksi)->first();
