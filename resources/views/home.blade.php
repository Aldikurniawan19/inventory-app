...
<div class="row mb-4">
    <div class="col-12">
        <h5 class="text-muted text-center">Analytics Dashboard Period: 01 Jan {{ date('Y') }} – 31 Dec {{ date('Y') }}</h5>
    </div>
</div>
        ...
        <div class="card card-stats card-round h-100"> 
            ...
            <div class="card-body">
                        ...
                        <tbody>
                            @forelse ($lowStockProducts as $index => $item)
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge {{ $item->stk_varian <= 0 ? 'bg-black' : 'bg-warning' }}">
                                            {{ $item->stk_varian }} Unit
                                        </span>
                                    </td>
                                </tr>
                            @empty
                ...
                <div class="text-center mt-3">
                    <a href="{{ route('master-data.stok-barang.index') }}" class="btn btn-outline-danger btn-sm">Lihat Selengkapnya</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            @if($hasIncomeExpenseData)
                ...
                var chartRevExp = new ApexCharts(document.querySelector("#revenueExpenseChart"), optionsRevenueExpense);
                chartRevExp.render();
            @endif
            ...
            @if($hasBestSellingData)
                ...
                var chartBestSelling = new ApexCharts(document.querySelector("#bestSellingChart"), optionsBestSelling);
            ...
            @if($hasPriceIncreaseData)
                ...
                var priceNames = priceIncreaseData.map(item => {
                    let name = (item.varian && item.varian.produk) ? item.varian.produk.nama_produk + ' ' + item.varian.nama_varian : 'Item #' + item.id;
                ...
                var chartPriceIncrease = new ApexCharts(document.querySelector("#priceIncreaseChart"), optionsPriceIncrease);
