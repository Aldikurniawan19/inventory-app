...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('item_stok_opnames', function (Blueprint $table) {
            $table->id();
            $table->foreignId('priode_stok_opname_id')->nullable()->constrained('priode_stok_opnames')->cascadeOnDelete()->cascadeOnUpdate();
            $table->string('nomor_sku');
            $table->integer('jumlah_stok')->nullable();
            $table->integer('jumlah_dilaporkan')->nullable();
            $table->enum('status', ['sesuai', 'selisih', 'belum_dilaporkan'])->default('belum_dilaporkan');
